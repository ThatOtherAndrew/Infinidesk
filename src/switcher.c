...
void switcher_start(struct infinidesk_switcher *switcher) {
    ...
    switcher->dirty = true;
    wlr_log(WLR_DEBUG, "Switcher started, selected view %p", (void *)switcher->selected);
}
...
void switcher_next(struct infinidesk_switcher *switcher) {
    ...
    switcher->dirty = true;
    wlr_log(WLR_DEBUG, "Switcher next, selected view %p", (void *)switcher->selected);
}
...
void switcher_prev(struct infinidesk_switcher *switcher) {
    ...
    switcher->dirty = true;
    wlr_log(WLR_DEBUG, "Switcher prev, selected view %p", (void *)switcher->selected);
}
...
void switcher_confirm(struct infinidesk_switcher *switcher) {
    ...
    if (switcher->selected) {
        ...
        wlr_log(WLR_DEBUG, "Switcher confirmed view %p", (void *)switcher->selected);
    }
...
static void render_texture(struct infinidesk_switcher *switcher) {
    ...
    PangoFontDescription *font_desc = pango_font_description_from_string(SWITCHER_FONT);
    ...
    wl_list_for_each(view, &server->views, link) {
        ...
        cairo_move_to(cr, SWITCHER_PADDING, item_y + (SWITCHER_ITEM_HEIGHT - 20) / 2);
    ...
    switcher->texture = wlr_texture_from_pixels(
        server->renderer,
        DRM_FORMAT_ARGB8888,
        stride,
        width,
    ...
    switcher->texture_width = width;
    switcher->texture_height = height;
    switcher->dirty = false;
...
void switcher_render(struct infinidesk_switcher *switcher,
    ...
    /* Re-render texture if dirty */
    if (switcher->dirty || !switcher->texture) {
        render_texture(switcher);
    }
