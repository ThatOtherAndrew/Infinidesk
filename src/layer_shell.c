...
void handle_new_layer_surface(struct wl_listener *listener, void *data) {
     ...
     * Layer surfaces are rendered at fixed screen positions, not in canvas
     * space.
     */
...
static void handle_layer_surface_commit(struct wl_listener *listener,
    ...
    if (layer_surface->initial_commit) {
        ...
        if (current_layer != pending_layer &&
            ...
            wl_list_insert(&layer->output->layer_surfaces[current_layer],
                           &layer->link);
        }
    ...
    if (committed & WLR_LAYER_SURFACE_V1_STATE_LAYER) {
        ...
        if (new_layer < LAYER_SHELL_LAYER_COUNT) {
            ...
            struct wlr_scene_tree *new_parent =
                layer->output->layer_trees[new_layer];
            wlr_scene_node_reparent(&layer->scene_tree->node, new_parent);

            /* Add to new list */
            wl_list_insert(&layer->output->layer_surfaces[new_layer],
                           &layer->link);
